...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a JDC address (e.g. %1)").arg("BST6R83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no jundcoin: URI
...
{
    // Convert jundcoin:// to jundcoin:
    //
    //    Cannot handle this later, because jundcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open jundcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Jundcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Jundcoin.lnk
...
{
    return GetAutostartDir() / "jundcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a jundcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Jundcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the jundcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add jundcoin app to startup item list
