...
#if defined(NDEBUG)
#error "Jundcoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
             if (nHeight >=      1 && nHeight <=      1) {nSubsidy = 400000.00 * COIN;}
        else if (nHeight >=      2 && nHeight <=    500) {nSubsidy =     10 * COIN;}
        ...
        else if (nHeight >=   5001                     ) {nSubsidy =       2 * COIN;}
    ...
    /* block rewards. */
         ...
         if (nHeight >=       1 && nHeight <=       1) {nSubsidy =  4000000 * COIN;}
    else if (nHeight >=       2 && nHeight <=   5000) {nSubsidy =      2 * COIN;}
    else if (nHeight >=   5001 && nHeight <=   25000) {nSubsidy =      4.5 * COIN;}
    else if (nHeight >=   25001 && nHeight <=   50000) {nSubsidy =     6.5 * COIN;}
    else if (nHeight >=   50001 && nHeight <=   75000) {nSubsidy =     8.5 * COIN;}
    else if (nHeight >=   75001 && nHeight <=   100000) {nSubsidy =    10 * COIN;}
    else if (nHeight >=   100001 && nHeight <=  200000) {nSubsidy =    9.5 * COIN;}
    else if (nHeight >=   200001 && nHeight <=  300000) {nSubsidy =     9 * COIN;}
    else if (nHeight >=  300001 && nHeight <=   400000) {nSubsidy =     8 * COIN;}
    else if (nHeight >=  400001 && nHeight <=   500000) {nSubsidy =     7 * COIN;}
    else if (nHeight >=  500001 && nHeight <=   600000) {nSubsidy =     6 * COIN;}
    else if (nHeight >=  600001 && nHeight <=   700000) {nSubsidy =     5 * COIN;}
    else if (nHeight >=  700001 && nHeight <=   800000) {nSubsidy =     4 * COIN;}
    else if (nHeight >=  800001 && nHeight <=   900000) {nSubsidy =     3 * COIN;}
    else if (nHeight >=  900001 )                      {nSubsidy =      2 * COIN;}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        ...
        else if (nHeight >=    501 && nHeight <=    100000) {nSubsidy = blockValue * 0.70;}
        else if (nHeight >= 100001 && nHeight <=    700000) {nSubsidy = blockValue * 0.65;}
        else if (nHeight >= 700001                       ) {nSubsidy = blockValue * 0.80;}
...
{
    RenameThread("jundcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // JUNDCOIN: We use certain sporks during IBD, so check to see if they are
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH (const CInv& inv, pto->vInventoryToSend) {
                ...
                if (inv.type == MSG_TX && !fSendTrickle) {
                    // 1/4 of tx invs jdc to all immediately
                    static uint256 hashSalt;
                    if (hashSalt == 0)
                        hashSalt = GetRandHash();
