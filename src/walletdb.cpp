...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Jundcoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite jundcoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> jundcoindData;
    bool allOK = dbenv.Jundcoin(newFilename, true, jundcoindData);
    if (jundcoindData.empty()) {
        LogPrintf("Jundcoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Jundcoin(aggressive) found %u records\n", jundcoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, jundcoindData) {
        if (fOnlyKeys) {
