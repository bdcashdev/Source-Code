...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("JundcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("JundcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("JundcoinMiner started (POS=%s)\n", (fProofOfStake ? "true" : "false") );
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jundcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running JundcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
